...
const diccionario = ["BAÑERA", "SAXOFON", "ARROZ", "CANCHA", "PELOTA", "PROGRAMA", "ORDENADOR", "MAGIA", "SOLDADO", "BAILE", "PLAZA", "EDIFICIO", "CUENCO", "ALBACETE", "TROMPETA", "SESGAR"];
...
function pantallaInicial() {
    ...
    letras.forEach(Element => { Element.style.cssText = "font-family: 'Rubik', sans-serif; color:#fc8403"; Element.disabled = false });
    document.getElementById("body").style.cssText = "background-image: linear-gradient(white, #f2ca72);";
        ...
        if (span[i].innerHTML == "") {
            ...
            for (let j = 0; j < letras.length; j++) {
                if (letras[j].innerHTML.toUpperCase() == span[i].innerHTML.toUpperCase()) {
                    letras[j].style.cssText = "color:rgba(255,0,0)";
                }
            }
        }
    }
